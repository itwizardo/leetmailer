...
def show_banner():
    ...
    dMP     dMMMMMP dMMMMMP dMMMMMMP .dMMMb  dMMMMMMMMb dMMMMMMP dMMMMb  .aMMMb  dMP dMP dMMMMMP .aMMMb  dMP dMP dMMMMMP dMMMMb 
   dMP     dMP     dMP        dMP   dMP" VP dMP"dMP"dMP   dMP   dMP.dMP dMP"VMP dMP dMP dMP     dMP"VMP dMP.dMP dMP     dMP.dMP 
  dMP     dMMMP   dMMMP      dMP    VMMMb  dMP dMP dMP   dMP   dMMMMP" dMP     dMMMMMP dMMMP   dMP     dMMMMK" dMMMP   dMMMMK"  
 dMP     dMP     dMP        dMP   dP .dMP dMP dMP dMP   dMP   dMP     dMP.aMP dMP dMP dMP     dMP.aMP dMP"AMF dMP     dMP"AMF   
dMMMMMP dMMMMMP dMMMMMP    dMP    VMMMP" dMP dMP dMP   dMP   dMP      VMMMP" dMP dMP dMMMMMP  VMMMP" dMP dMP dMMMMMP dMP dMP                                                                                                                                  
                                                                                                                                                                   
                {b}Leetsmtpchecker v1.05.24{z}
                Made by {b}Itwizardo{z}
	"""
...
def smtp_connect_and_send(smtp_server, port, login_template, smtp_user, password):
	...
	if answer[:3] == '220':
		...
		if not verify_email:
			...
			return True
		headers_arr = [
			'From: Leet checker <%s>'%smtp_user,
			'Resent-From: admin@localhost',
			'To: '+verify_email,
			'Subject: new SMTP from Leetchecker checker',
...
check_ipv6()
try:
	help_message = f'usage: \n{npt}python3 leetchecker.py '+bold('list.txt')+' [verify_email@example.com] [ignored,email,domains] [start_from_line] [debug]'
